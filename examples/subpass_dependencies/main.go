...
func main() {
	...
	readbackBufferSize := vulkan.DeviceSize(renderWidth * renderHeight * 4)
	readbackBuffer, err := vulkan.CreateBuffer(device, &vulkan.BufferCreateInfo{
		Size:        readbackBufferSize,
		Usage:       vulkan.BufferUsageTransferDstBit,
		SharingMode: vulkan.SharingModeExclusive,
	})
	if err != nil {
		log.Fatalf("Failed to create readback buffer: %v", err)
	}
	defer vulkan.DestroyBuffer(device, readbackBuffer)

	readbackMemReqs := vulkan.GetBufferMemoryRequirements(device, readbackBuffer)
	readbackMemTypeIndex, found := vulkan.FindMemoryType(memProps, readbackMemReqs.MemoryTypeBits,
		vulkan.MemoryPropertyHostVisibleBit|vulkan.MemoryPropertyHostCoherentBit)
	if !found {
		log.Fatalf("Failed to find memory type for readback buffer")
	}

	readbackBufferMemory, err := vulkan.AllocateMemory(device, &vulkan.MemoryAllocateInfo{
		AllocationSize:  readbackMemReqs.Size,
		MemoryTypeIndex: readbackMemTypeIndex,
	})
	if err != nil {
		log.Fatalf("Failed to allocate readback buffer memory: %v", err)
	}
	defer vulkan.FreeMemory(device, readbackBufferMemory)

	if err := vulkan.BindBufferMemory(device, readbackBuffer, readbackBufferMemory, 0); err != nil {
		log.Fatalf("Failed to bind readback buffer memory: %v", err)
	}
	fmt.Println("   ✓ Readback buffer created and bound")
	...
	renderPass, err := vulkan.CreateRenderPass(device, &vulkan.RenderPassCreateInfo{
		...
		Subpasses: []vulkan.SubpassDescription{
			...
			{
				// Subpass 1: post-process (just for demonstration)
				PipelineBindPoint: vulkan.PipelineBindPointGraphics,
				ColorAttachments: []vulkan.AttachmentReference{
					{Attachment: 0, Layout: vulkan.ImageLayoutColorAttachmentOptimal},
				},
		...
		Dependencies: []vulkan.SubpassDependency{
			{
				// External dependency before Subpass 0
				...
				DependencyFlags: vulkan.DependencyByRegionBit,
			},
			{
				// Dependency between Subpass 0 and Subpass 1
				SrcSubpass:      0,
				DstSubpass:      1,
				SrcStageMask:    vulkan.PipelineStageColorAttachmentOutputBit,
				DstStageMask:    vulkan.PipelineStageFragmentShaderBit, // Assuming next pass reads it
				SrcAccessMask:   vulkan.AccessColorAttachmentWriteBit,
				DstAccessMask:   vulkan.AccessShaderReadBit,
				DependencyFlags: vulkan.DependencyByRegionBit,
			},
			{
				// Self-dependency within Subpass 1 (e.g., read/write to same attachment)
				SrcSubpass:      1,
				DstSubpass:      1,
				SrcStageMask:    vulkan.PipelineStageColorAttachmentOutputBit,
				DstStageMask:    vulkan.PipelineStageColorAttachmentOutputBit,
				SrcAccessMask:   vulkan.AccessColorAttachmentWriteBit,
				DstAccessMask:   vulkan.AccessColorAttachmentWriteBit | vulkan.AccessColorAttachmentReadBit,
				DependencyFlags: vulkan.DependencyByRegionBit,
			},
			{
				// External dependency after Subpass 1
				SrcSubpass:      1,
				DstSubpass:      vulkan.SubpassExternal,
				SrcStageMask:    vulkan.PipelineStageColorAttachmentOutputBit,
				DstStageMask:    vulkan.PipelineStageTransferBit,
				SrcAccessMask:   vulkan.AccessColorAttachmentWriteBit,
	...
	fmt.Println("   ✓ Render pass created with subpasses and dependency chains")
	...
	// Create a second pipeline for subpass 1 (just re-using the same state for demo)
	graphicsPipelineCreateInfo2 := graphicsPipelineCreateInfo
	graphicsPipelineCreateInfo2.Subpass = 1

	pipelines, err := vulkan.CreateGraphicsPipelines(device, vulkan.PipelineCache(nil),
		[]vulkan.GraphicsPipelineCreateInfo{graphicsPipelineCreateInfo, graphicsPipelineCreateInfo2})
	if err != nil {
		log.Fatalf("Failed to create graphics pipelines: %v", err)
	}
	graphicsPipeline := pipelines[0]
	graphicsPipeline2 := pipelines[1]
	defer vulkan.DestroyPipeline(device, graphicsPipeline)
	defer vulkan.DestroyPipeline(device, graphicsPipeline2)
	fmt.Println("   ✓ Graphics pipelines created for subpasses 0 and 1")
	...
	// Draw the triangle (3 vertices, 1 instance) for subpass 0
	vulkan.CmdDraw(cb, uint32(len(triangleVertices)), 1, 0, 0)

	// Move to next subpass
	vulkan.CmdNextSubpass(cb, vulkan.SubpassContentsInline)

	// Bind the second pipeline
	vulkan.CmdBindPipeline(cb, vulkan.PipelineBindPointGraphics, graphicsPipeline2)

	// Draw the triangle for subpass 1
	vulkan.CmdDraw(cb, uint32(len(triangleVertices)), 1, 0, 0)
	...
	vulkan.CmdCopyImageToBuffer(cb, image, vulkan.ImageLayoutTransferSrcOptimal, readbackBuffer, []vulkan.BufferImageCopy{
		{
			BufferOffset:      0,
			BufferRowLength:   0,
			BufferImageHeight: 0,
			...
			ImageExtent: vulkan.Extent3D{Width: renderWidth, Height: renderHeight, Depth: 1},
		},
	})
	...
	// -----------------------------------------------------------------------
	// 15. Readback and verify
	// -----------------------------------------------------------------------
	fmt.Println("\n15. Reading back pixels...")
	mappedData, err := vulkan.MapMemory(device, readbackBufferMemory, 0, readbackBufferSize, 0)
	if err != nil {
		log.Fatalf("Failed to map readback buffer memory: %v", err)
	}
	pixels := unsafe.Slice((*byte)(mappedData), int(readbackBufferSize))

	// Print a few pixels
	getPixel := func(x, y int) (r, g, b, a byte) {
		idx := (y*renderWidth + x) * 4
		return pixels[idx], pixels[idx+1], pixels[idx+2], pixels[idx+3]
	}

	tlR, tlG, tlB, tlA := getPixel(0, 0)
	cR, cG, cB, cA := getPixel(renderWidth/2, renderHeight/2)

	fmt.Printf("   Top-Left pixel (background): R:%d G:%d B:%d A:%d\n", tlR, tlG, tlB, tlA)
	fmt.Printf("   Center pixel (triangle):     R:%d G:%d B:%d A:%d\n", cR, cG, cB, cA)

	vulkan.UnmapMemory(device, readbackBufferMemory)
	...
	fmt.Println("  ✓ Image to Buffer copy (CmdCopyImageToBuffer)")
	fmt.Println("  ✓ Host-visible readback buffer mapping")
}
