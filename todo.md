...
- [ ] **Clean up `benchmark_backup/`** — Either merge useful parts into the main `benchmark/` example or remove the backup directory to reduce duplication.
...
- [ ] **Add secondary command buffer support** — `CommandBufferInheritanceInfo` is defined but secondary command buffer recording and execution (`CmdExecuteCommands`) need examples and tests.
...
- [ ] **Pool allocator for descriptor sets** — Provide a higher-level descriptor set pool manager that reduces allocation overhead.
